...
import {
  useConfirmCarReturn,
  useConfirmDamagePayment,
  useConfirmRentPayment,
  useGetDamagePayments,
...
const Payments = () => {
  ...
  const [selectedDamagePayment, setSelectedDamagePayment] = useState([]);
  ...
  const { data: damagePayments, isLoading: damagePaymentLoading } =
    useGetDamagePayments({ paymentStatus: paymentStatus });
  ...
  const { mutate: confirmCarReturn, isLoading: confirmReturnLoading } =
    useConfirmCarReturn();
  const { mutate: confirmDamagePayment, isLoading: confirmingDamagePayment } =
    useConfirmDamagePayment();
  ...
  useEffect(() => {
    if (damagePayments) {
      setSelectedDamagePayment(damagePayments?.data?.payments[0]);
    }
  }, [damagePayments]);
  ...
  return (
    <div className="bookings">
      ...
      {showDamageConfirmModal && (
        <div className="modal-overlay">
          <div className="modal text-white w-1/3">
            <div className="modal-header">
              <h2 className="text-2xl font-bold">Confirm Payment For Damage</h2>
              <button
                className="close-button"
                onClick={() => setShowDamageConfirmModal(false)}>
                &times;
              </button>
            </div>
            <div className="modal-content">
              <span>Confirm this if the damage payment has been received.</span>
            </div>
            <button
              onClick={() =>
                confirmDamagePayment(
                  {
                    paymentId: selectedDamagePayment?.id,
                    userId: JSON.parse(token)?.user?.id,
                  },
                  {
                    onSuccess: () => setShowDamageConfirmModal(false),
                  }
                )
              }
              className="bg-slate-500 py-2 px-4"
              disabled={damagePaymentLoading}
              style={{
                border: "none",
                borderRadius: 20,
                color: "white",
              ...
              {damagePaymentLoading ? "Loading..." : "Confirm"}
            </button>
          </div>
        </div>
      )}
      ...
      {active === "damagePayment" && (
        <Wrapper>
          <div className="grid-four-column">
            {damagePayments?.data?.payments?.map((curElm, index) => {
              return (
                <Card
                  key={index}
                  className={`px-4 pt-2 rounded-md w-full hover:bg-slate-400 transition-colors ease-in-out duration-300 mb-4
          ...
          <div className="main-screen h-fit">
            {selectedDamagePayment && (
              <Card className="w-full px-4 py-4 text-white">
                <div className="d-flex align-items-center ">
                  <div className="mt-4">
                    <span style={{ fontWeight: "300" }}>
                      ID: {selectedDamagePayment?.id}
                    </span>
                  </div>
                </div>
                <div className="mt-8 w-full">
                  ...
                  <div className="flex mt-2 justify-between">
                    <span>Date:</span>
                    <span className="font-bold">
                      {selectedDamagePayment?.paymentStatus === "Paid"
                        ? formatDate(selectedDamagePayment?.paymentDate)
                        : "N/A"}
                    </span>
                  </div>
                  <div className="flex mt-2 justify-between">
                    <span>Amount:</span>
                    <span className="font-bold">
                      {selectedDamagePayment?.amount}
                    </span>
                  </div>
                  <div className="flex mt-2 justify-between">
                    <span>Status:</span>
                    <span className="font-bold">
                      {selectedDamagePayment?.paymentStatus}
                    </span>
                  </div>
                  <div className="flex mt-2 justify-between">
                    <span>Payment Type:</span>
                    <span className="font-bold">
                      {selectedDamagePayment?.paymentType || "N/A"}
                    </span>
                  </div>
                  <div className="flex mt-2 justify-between">
                    <span>Damage Approved By:</span>
                    <span className="font-bold">
                      {selectedDamagePayment?.checkedBy?.name}
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-8 w-full">
                  ...
                  <div className="flex mt-2 justify-between">
                    <span>Car Name:</span>
                    <span className="font-bold">
                      {selectedDamagePayment?.rentalDetails?.car?.name}
                    </span>
                  </div>
                  <div className="flex mt-2 justify-between">
                    <span>Customer Name:</span>
                    <span className="font-bold">
                      {selectedDamagePayment?.rentalDetails?.customer?.name}
                    </span>
                  </div>
                </div>

                <div className="mt-8">
                  {selectedDamagePayment?.paymentStatus === "Pending" && (
                    <button
                      onClick={() => setShowDamageConfirmModal(true)}
                      className="bg-slate-500 py-2 px-4"
                      style={{
                        border: "none",
                        borderRadius: 20,
